...
func errorHandler(w http.ResponseWriter, r *http.Request, path string) {
	mu.Lock()
	defer mu.Unlock()

	// Pass the artists slice to the template
	err := templates.ExecuteTemplate(w, "error.html", fmt.Sprintf("%s does not exist", path))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}
